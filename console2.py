...
try:
    ...
    logo_max_width = WIDTH // 2
    ratio = logo_max_width / logo_img.get_width()
    new_size = (
        int(logo_img.get_width() * ratio),
        int(logo_img.get_height() * ratio)
    )

    logo_img = pygame.transform.smoothscale(logo_img, new_size)
...
while running:
    ...
    if state == "menu":
        ...
        if logo_img:
            screen.blit(
                logo_img,
                (WIDTH//2 - logo_img.get_width()//2, 100)
            )
        ...
        # PROPAGANDA
        sponsor = small_font.render(
            "Apoiador: Seu Nome Aqui",
            True,
            (200, 200, 50)
        )
        screen.blit(
            sponsor,
            (WIDTH//2 - sponsor.get_width()//2, HEIGHT - 100)
        )
